...
| Generation-set history | Tampering | Edit/review a superseded case or let a failed run become active | Historical evidence changes or active coverage becomes inconsistent | One deterministic successful-run resolver, active-only mutations/default reads, explicit confirmation before superseding reviewed/revised evidence | Consider a materialized active-set pointer only with reconciliation evidence |
