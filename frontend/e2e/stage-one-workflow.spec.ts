...
test('complete generation workflow validates input, resists injected instructions, reviews, and exports', async ({
  ...
  const dialog = page.getByRole('dialog', { name: 'Add a requirement' });
  await dialog.getByRole('button', { name: 'Save requirement' }).click();
  await expect(dialog.getByText('Enter a title.')).toBeVisible();
  await expect(dialog.getByText('Enter the user story or requirement.')).toBeVisible();
  await expect(dialog.getByText('Describe the expected behavior.')).toBeVisible();

  await dialog.getByLabel('Requirement title').fill('Prevent duplicate checkout submission');
  await dialog
    .getByLabel('User story or requirement')
    .fill(
  ...
  await dialog
    .getByLabel('Business rules and constraints')
    .fill(
...
test('provider failure is safe and retry does not retain a partial generation', async ({
  ...
  await page.getByRole('button', { name: 'New requirement' }).click();
  const dialog = page.getByRole('dialog', { name: 'Add a requirement' });
  await dialog.getByLabel('Requirement title').fill('Preserve a safe retry boundary');
  await dialog
    .getByLabel('User story or requirement')
    .fill('As a QA analyst, I want a failed generation to leave no partial test cases.');
  await dialog
    .getByLabel('AC-1')
    .fill('A failed provider request leaves the requirement with zero generated cases.');
  await dialog.getByRole('button', { name: 'Save requirement' }).click();
  await expect(
  ...
  await page.route('**/api/v1/requirements/*/generate-test-cases', async (route) => {
    await route.fulfill({
      status: 503,
  ...
  await page.unroute('**/api/v1/requirements/*/generate-test-cases');
  await page.getByRole('button', { name: 'Generate tests' }).click();
  await expect(
    page.getByText('Generation completed and passed the server-side quality gate.'),
  ).toBeVisible();
