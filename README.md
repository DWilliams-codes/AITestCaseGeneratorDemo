...
Prerequisites: Java 21, Maven 3.9+, Node.js 24, and npm 11.
...
The demo contains a Customer Returns Portal project, one realistic return requirement, seven structured test cases, three approvals, ambiguities, traceability, and exportable evidence. The demo profile uses a local H2 file and fixed non-production credentials; never enable it in a shared or production environment.
...
# Replace POSTGRES_PASSWORD and JWT_ACCESS_TOKEN_SECRET in .env.
...
| `TEST_GENERATION_PROVIDER` | No | `fake` | `fake` or `openai` |
| `OPENAI_API_KEY` | OpenAI mode | none | Server-side provider key |
| `OPENAI_MODEL` | No | `gpt-5-mini-2025-08-07` | Pinned provider model |
